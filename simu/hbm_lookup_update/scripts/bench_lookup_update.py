...
def main():
    ...
    table_states_cpu = (table_keys_cpu.astype(np.int64) * 3 + 1).astype(np.int32)
    query_cpu = rng.choice(np.arange(table_size, dtype=np.int32), size=query_len, replace=True).astype(np.int32)
    new_states_cpu = (777000 + np.arange(query_len, dtype=np.int32)).astype(np.int32)
    ...
    lookups = iters * query_len
    updates = iters * ((query_len * update_percent) // 100)
    print(f"query_len={query_len} block_dim={block_dim} update_percent={update_percent}")
