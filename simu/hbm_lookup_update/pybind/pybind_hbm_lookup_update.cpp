    ...
    bool doUpdate = true)
{
    ...
    TORCH_CHECK(tableStates.numel() == TABLE_SIZE,
                "table_states must have exactly 2048 elements");
    TORCH_CHECK(queryKeys.dim() == 1, "query_keys must be 1-D");
    ...
    TORCH_CHECK(queryKeys.numel() <= static_cast<int64_t>(UINT32_MAX),
                "query_keys is too large for this demo kernel");

    int64_t queryLen64 = queryKeys.numel();
    int64_t paddedLen64 = AlignUpI64(std::max<int64_t>(queryLen64, 1), QUERY_TILE);
    auto outFull = at::empty({paddedLen64}, queryKeys.options());
    ...
    return outFull.narrow(0, 0, queryLen64);
}
